...
namespace SerialPortW
{
    public partial class SerialPortW : Form
    {
        ...
        private void portInit()
        {
            ...
            TextBoxMatchString.Text = "abcdefghijklmnopqrstuvwxyz0123456789";
        ...
        private void dealReceive(object sender, SerialDataReceivedEventArgs e)
        {
            if (serialPort.IsOpen == true)
            {
                ...
                {
                    // 串口关闭了，但是缓存中还有数据，读取会出现异常,只在关闭的时候出现，后面再处理
        ...
        private void SetText(String text)
        {
            ...
            // 在末尾添加新内容
            richTextBoxReceive.AppendText(text + ' ' + lineNum + '\n');
            //Console.WriteLine("text {0}-----", text);

            //正则表达式，进行匹配
            String regex = TextBoxMatchString.Text.ToString().Trim();
            //Console.WriteLine("rege {0}-----", regex);
            if (regex != null)
            {
                //正则表达式进行比较
                ...
                {
                    matchNum++;
                    labelMatchNum.Text = matchNum.ToString();
                ...
                else
                {
                    //不匹配
                    mismatchNum++;
                    labelMismatchNum.Text = mismatchNum.ToString();
        ...
        private void buttonOpen_Click(object sender, EventArgs e)
        {
            if (buttonOpen.Text == "Open")
            {
                try
                {
                    ...
                    this.labelMatchNum.Text = matchNum.ToString();
                    this.labelMismatchNum.Text = mismatchNum.ToString();
...
//                serialPort.Write('^C');
        ...
        void tmr_timeout(object sender, ElapsedEventArgs e)
        {
            if (serialPort != null && serialPort.IsOpen)
            {
                ...
                {
                    //可能串口已经关闭，但是再write的话，就会出现异常，这一块还没处理
